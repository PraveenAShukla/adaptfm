...
We started with a trained drafter, spent a week on n-gram lookup, moved to suffix
decoding for the speed, and came back to DFlash at the end when suffix could not clear
the quality gate. The ranked entry uses `K=15`, which is not among the tags above. It
was exported and uploaded as a tarball rather than pushed, so the closest published
relatives are the `dflash-r6` images: same configuration, 20 draft tokens instead of 15.
...
We had reused tag names across rebuilds, and by the end we could not tell from a tag what
was inside the image. Name images after the configuration, or record the digest when you
build.

## What did not work, and why

**n-gram lookup, roughly ten days.** It drafts by copying spans from the prompt, so it
looks excellent on a benchmark with repetitive input and falls off on generated text.
We measured around 13.8× at depth 40 on the A10G, but the startup cost climbed with
...
algorithm at the same time, so it says nothing about the best `K` for either method. What
it does show is that rejected draft tokens still cost drafting and verification time, so
